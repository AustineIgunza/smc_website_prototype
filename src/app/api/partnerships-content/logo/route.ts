...
export async function POST(req: NextRequest) {
  ...
  const partnerId = (formData.get("partnerId") as string) || "partner";
  ...
  // Generate a unique file path under partnerships folder
  const ext = file.name.split(".").pop() || "png";
  const cleanId = partnerId.replace(/[^a-z0-9-_]/gi, "-").toLowerCase();
  const filePath = `partnerships/${cleanId}-${Date.now()}.${ext}`;
