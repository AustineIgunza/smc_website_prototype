...
export async function POST(req: NextRequest) {
  ...
  const stepNum = (formData.get("stepNum") as string) || "step";
  ...
  // Generate a unique file path under membership folder in the bucket
  const ext = file.name.split(".").pop() || "png";
  const filePath = `membership/step-${stepNum}-${Date.now()}.${ext}`;
